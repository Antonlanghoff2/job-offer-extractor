...
from src.cv_dataset_generator import CONTACT_LABELS, _write_jsonl, generate_dataset
from src.validate_cv_dataset import format_report, main as validate_main, validate_records
...
def test_validator_cli_exit_code(tmp_path: Path) -> None:
    records = generate_dataset(count=2, seed=42)
    input_path = tmp_path / "cv.jsonl"
    _write_jsonl(input_path, records)
    assert validate_main(["--input", str(input_path)]) == 0
