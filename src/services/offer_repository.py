...
def load_normalized_offers(path: Path = DEFAULT_RAW_OFFERS_PATH) -> tuple[list[dict[str, Any]], str | None]:
...
def _territory_label(territory: str | None) -> str:
    if not territory:
        return "Tous les territoires"
    return territory


def _period_label(offers: Iterable[dict[str, Any]]) -> str | None:
    dates = [
        parsed
